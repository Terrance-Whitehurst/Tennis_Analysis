"""
Launch RF-DETR tennis ball detection training on AWS SageMaker.
...
Prerequisites:
    ...
    3. Dataset at data/raw/Tennis_Ball_Detection/ (or already on S3)
    4. pip install sagemaker boto3

Usage:
    # Basic launch (uploads data, starts training)
    python scripts/sagemaker/launch_ball_detection.py \
        --role arn:aws:iam::123456789012:role/SageMakerRole

    # Use data already on S3
    python scripts/sagemaker/launch_ball_detection.py \
        --role arn:aws:iam::123456789012:role/SageMakerRole \
        --s3_data s3://my-bucket/datasets/Tennis_Ball_Detection
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Launch RF-DETR ball detection training on SageMaker"
    )
    ...
    parser.add_argument(
        "--s3_prefix",
        type=str,
        default="tennis-analysis/datasets/Tennis_Ball_Detection",
...
def main():
    ...
    # S3 output path
    s3_output = args.s3_output or f"s3://{bucket}/tennis-analysis/models/ball_detection"

    # Job name
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    job_name = args.job_name or f"rfdetr-ball-{args.model}-{timestamp}"
    ...
    tags.append({"Key": "task", "Value": "ball-detection"})

    # Spot instance config
    checkpoint_s3 = (
        f"s3://{bucket}/tennis-analysis/checkpoints/ball_detection/"
        if args.spot
        else None
    )
    ...
    estimator = PyTorch(
        entry_point="entry_ball_detection.py",
