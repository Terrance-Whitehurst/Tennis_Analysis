"""Interactive Dash web UI for manual label correction."""
        ...
        html.Div(
            children=[
                html.Button(
                    ...
                    id="write-btn",
                    n_clicks=0,
                    style={"width": "160px", "height": "40px", "margin": "10px"},
                ),
                html.Button(
                    "Reset Label",
                    id="reset-btn",
...
@app.callback(
    ...
    [Input("eval-file-dropdown", "value"), Input("rally-dropdown", "value")],
)
def change_dropdown(eval_file, rally_key):
    ...
    x_gt, y_gt, vis_gt = (
        np.array(gt_df["X"]),
        np.array(gt_df["Y"]),
        np.array(gt_df["Visibility"]),
    )

    # Init correct result
    x_correct, y_correct, vis_correct = (
        np.array(gt_df["X"]),
        np.array(gt_df["Y"]),
        np.array(gt_df["Visibility"]),
    )
    ...
    x_pred, y_pred, vis_pred = (
        np.array(eval_dict["X"]),
        np.array(eval_dict["Y"]),
        np.array(eval_dict["Visibility"]),
    )
...
@app.callback(Output("write-mag", "value"), Input("write-btn", "n_clicks"))
...
def show_frame(hoverData, clickData, n_clicks):
    ...
    if prev_click != n_clicks:
        ...
        frame_fig.add_trace(
            go.Scatter(
                x=x_pred[frame_id : frame_id + 1],
                y=y_pred[frame_id : frame_id + 1],
