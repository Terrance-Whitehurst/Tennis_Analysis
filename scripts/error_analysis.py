"""Dash dashboard for detailed error analysis."""
...
from src.utils.general import IMG_FORMAT, get_rally_dirs
...
def change_dropdown(eval_file_1, eval_file_2, rally_key):
    ...
    x_pred_1, y_pred_1, vis_pred_1 = (
        np.array(eval_dict_1["X"]),
        np.array(eval_dict_1["Y"]),
        np.array(eval_dict_1["Visibility"]),
    )
    x_pred_2, y_pred_2, vis_pred_2 = (
        np.array(eval_dict_2["X"]),
        np.array(eval_dict_2["Y"]),
        np.array(eval_dict_2["Visibility"]),
    )
    ...
    time_fig = go.Figure().set_subplots(
        rows=2,
        cols=1,
        shared_xaxes=True,
        vertical_spacing=0.15,
        subplot_titles=(eval_file_1, eval_file_2),
    )
    for i, showlegend in [(0, True), (1, False)]:
        for pred_type in pred_types:
            time_fig.add_trace(
                go.Bar(
                    x=timestamp,
                    y=bar_list[i][pred_type],
                    customdata=hover_data,
                    width=bar_width,
                    marker_color=colors[pred_type],
                    name=pred_type,
                    legendgroup=pred_type,
        ...
        if split == "test":
            # The moment of serve
            time_fig.add_vline(
                x=start_f[rally_key] - bar_width / 2,
                line_width=1,
                line_dash="dash",
                line_color="gray",
                row=i + 1,
    ...
    time_fig.update_layout(
        ...
        height=300,
        title_text=f"Rally {rally_key} Error Distribution",
        title_x=0.5,
    )
...
def show_frame(hoverData):
    global match_id, rally_id, x_gt, y_gt, x_pred_1, y_pred_1, x_pred_2, y_pred_2
    traj_len = 16
    _radius, _bbox_width, marker_size = 5, 1, 5
    ...
    frame_fig.add_trace(
        go.Scatter(
            x=x_pred_1[frame_id - traj_len + 1 : frame_id + 1],
    ...
    frame_fig.add_trace(
        go.Scatter(
            x=x_pred_1[frame_id : frame_id + 1],
            y=y_pred_1[frame_id : frame_id + 1],
    ...
    frame_fig.add_trace(
        go.Scatter(
            x=x_pred_2[frame_id : frame_id + 1],
            y=y_pred_2[frame_id : frame_id + 1],
    ...
    frame_fig.add_trace(
        go.Scatter(
            x=x_gt[frame_id : frame_id + 1],
            y=y_gt[frame_id : frame_id + 1],
    ...
    frame_fig.update_layout(
        ...
        width=img_w,
        height=img_h,
        title_text=f"Frame {frame_id} label: ({cx}, {cy}), pred 1: ({cx_pred_1}, {cy_pred_1}), pred 2: ({cx_pred_2}, {cy_pred_2})",
        title_x=0.45,
    )
