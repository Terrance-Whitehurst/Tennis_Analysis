...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1800,
        help="maximum number of frames to sample for generating median image",
    )
    parser.add_argument(
        "--video_range",
        type=lambda splits: [int(s) for s in splits.split(",")],
        default=None,
        help="range of start second and end second of the video for generating median image",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        default="pred_result",
        help="directory to save the prediction result",
    )
    parser.add_argument(
        "--large_video",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="whether to output video with predicted trajectory",
    )
    parser.add_argument(
        "--traj_len", type=int, default=8, help="length of trajectory to draw on video"
    )
    ...
    if args.output_video:
        write_pred_video(
            video_file, pred_dict, save_file=out_video_file, traj_len=args.traj_len
        )
