...
from src.datasets.tracknet_dataset import Shuttlecock_Trajectory_Dataset, data_dir
...
def get_ensemble_weight(seq_len: int, eval_mode: str) -> torch.Tensor:
...
def predict_location(heatmap: npt.NDArray[np.uint8]) -> tuple[int, int, int, int]:
...
def evaluate(
    ...
    if c_true is not None and c_pred is not None:
        ...
        assert output_bbox is False, "Coordinate prediction cannot output detection"
...
def generate_inpaint_mask(pred_dict: dict[str, list], th_h: float = 30) -> list[int]:
...
def linear_interp(target: list, inpaint_mask: list) -> npt.NDArray:
    assert len(target) == len(inpaint_mask), (
...
def get_eval_res(pred_dict: dict[str, list]) -> npt.NDArray:
...
def eval_tracknet(
    model: nn.Module, data_loader: DataLoader, param_dict: dict[str, Any]
) -> tuple[float, dict]:
    ...
    for step, (i, x, y, _, _) in enumerate(data_prob):
        ...
        if param_dict["verbose"]:
            ...
            data_prob.set_description("Evaluation")
...
def get_coco_res(pred_dict: dict, drop: bool = False) -> list[dict]:
    """Parse prediction result and get evaluation result.
...
def get_test_res(pred_dict: dict, drop: bool = False) -> dict:
    """Parse prediction result and get evaluation result.
...
def test_rally_linear(model: tuple, rally_dir: str, param_dict: dict[str, Any]) -> dict:
    tracknet, _ = model
    w, h = Image.open(os.path.join(rally_dir, "0.png")).size
    w_scaler, h_scaler = w / WIDTH, h / HEIGHT
