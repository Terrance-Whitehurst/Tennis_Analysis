...
from src.datasets.tracknet_dataset import data_dir
...
def write_to_tb(
    ...
    if model_type == "TrackNet":
        tb_writer.add_scalars(
            f"{model_type}_Loss/WBCE", {"train": losses[0], "val": losses[1]}, epoch
        )
        tb_writer.add_scalar(f"{model_type}_Metric/Accurcy", val_res["accuracy"], epoch)
        tb_writer.add_scalar(
            f"{model_type}_Metric/Precision", val_res["precision"], epoch
        )
        ...
        tb_writer.add_scalar(
            f"{model_type}_Metric/Miss_Rate", val_res["miss_rate"], epoch
        )
        ...
        tb_writer.add_scalars(
            f"{model_type}_Loss/MSE", {"train": losses[0], "val": losses[1]}, epoch
        )
        tb_writer.add_scalars(
            f"{model_type}_Metric/Accurcy",
            {
                "val_refine": val_res["inpaint"]["accuracy"],
                "val_reconstruct": val_res["reconstruct"]["accuracy"],
                "val_baseline": val_res["baseline"]["accuracy"],
            },
            epoch,
        )
        tb_writer.add_scalars(
            f"{model_type}_Metric/Precision",
            {
                "val_refine": val_res["inpaint"]["precision"],
                "val_reconstruct": val_res["reconstruct"]["precision"],
                "val_baseline": val_res["baseline"]["precision"],
            },
            epoch,
        )
        tb_writer.add_scalars(
            f"{model_type}_Metric/Recall",
            {
                "val_refine": val_res["inpaint"]["recall"],
                "val_reconstruct": val_res["reconstruct"]["recall"],
                "val_baseline": val_res["baseline"]["recall"],
            },
            epoch,
        )
        tb_writer.add_scalars(
            f"{model_type}_Metric/F1-score",
            {
                "val_refine": val_res["inpaint"]["f1"],
                "val_reconstruct": val_res["reconstruct"]["f1"],
                "val_baseline": val_res["baseline"]["f1"],
            },
            epoch,
        )
        tb_writer.add_scalars(
            f"{model_type}_Metric/Miss_Rate",
            {
                "val_refine": val_res["inpaint"]["miss_rate"],
                "val_reconstruct": val_res["reconstruct"]["miss_rate"],
                "val_baseline": val_res["baseline"]["miss_rate"],
            },
            epoch,
        )
        tb_writer.add_scalars(
            f"{model_type}_Results/TP",
            {
                "val_refine": val_res["inpaint"]["TP"],
                "val_reconstruct": val_res["reconstruct"]["TP"],
                "val_baseline": val_res["baseline"]["TP"],
            },
            epoch,
        )
        tb_writer.add_scalars(
            f"{model_type}_Results/TN",
            {
                "val_refine": val_res["inpaint"]["TN"],
                "val_reconstruct": val_res["reconstruct"]["TN"],
                "val_baseline": val_res["baseline"]["TN"],
            },
            epoch,
        )
        tb_writer.add_scalars(
            f"{model_type}_Results/FP1",
            {
                "val_refine": val_res["inpaint"]["FP1"],
                "val_reconstruct": val_res["reconstruct"]["FP1"],
                "val_baseline": val_res["baseline"]["FP1"],
            },
            epoch,
        )
        tb_writer.add_scalars(
            f"{model_type}_Results/FP2",
            {
                "val_refine": val_res["inpaint"]["FP2"],
                "val_reconstruct": val_res["reconstruct"]["FP2"],
                "val_baseline": val_res["baseline"]["FP2"],
            },
            epoch,
        )
        tb_writer.add_scalars(
            f"{model_type}_Results/FN",
            {
                "val_refine": val_res["inpaint"]["FN"],
                "val_reconstruct": val_res["reconstruct"]["FN"],
                "val_baseline": val_res["baseline"]["FN"],
            },
            epoch,
        )
    tb_writer.flush()
...
def plot_median_files(data_dir: str) -> None:
    ...
    for split in ["train", "test", "val"]:
        ...
        for match_dir in match_dirs:
            ...
            if os.path.exists(
                os.path.join(data_dir, split, f"match{match_id}", "median.npz")
            ):
                median = np.load(
                    os.path.join(data_dir, split, f"match{match_id}", "median.npz")
                )["median"][..., ::-1]  # BGR to RGB
                cv2.imwrite(
                    os.path.join(
                        data_dir, "median", f"{split}_m{match_id}.{IMG_FORMAT}"
                    ),
                    median,
                )
                ...
                if os.path.exists(os.path.join(rally_dir, "median.npz")):
                    ...
                    cv2.imwrite(
                        os.path.join(
                            data_dir,
                            "median",
                            f"{split}_m{match_id}_r{rally_id}.{IMG_FORMAT}",
                        ),
                        median,
                    )


def plot_heatmap_pred_sample(
    x: npt.NDArray,
    y: npt.NDArray,
    y_pred: npt.NDArray,
    c: npt.NDArray,
    ...
    for f in range(c.shape[0]):
        ...
        tmp_x = (
            cv2.cvtColor(x[f], cv2.COLOR_GRAY2BGR) if bg_mode == "subtract" else x[f]
        )
        ...
        cv2.circle(
            tmp_x, (int(c[f][0] * WIDTH), int(c[f][1] * HEIGHT)), 2, (255, 0, 0), -1
        )
        ...
        imgs.append(img)
        imgs[0].save(
            f"{save_dir}/cur_pred_TrackNet.gif",
            format="GIF",
            save_all=True,
...
def plot_traj_pred_sample(
    coor_gt: npt.NDArray,
    coor_inpaint: npt.NDArray,
    inpaint_mask: npt.NDArray,
    save_dir: str = "",
) -> None:
    ...
    for f in range(coor_gt.shape[0]):
        img = cv2.circle(
            img,
            (int(coor_gt[f][0] * WIDTH), int(coor_gt[f][1] * HEIGHT)),
            2,
            (0, 0, 255),
            -1,
        )
        if inpaint_mask[f] == 1:
            img = cv2.circle(
                img,
                (int(coor_inpaint[f][0] * WIDTH), int(coor_inpaint[f][1] * HEIGHT)),
                2,
                (0, 255, 0),
                -1,
            )
...
def plot_diff_hist(
    pred_dict_base: dict, pred_dict_refine: dict, split: str, save_dir: str
) -> None:
    ...
    for err_type in ["FP1", "FP2"]:
        ...
        for rally_key in rally_keys:
            ...
            csv_file = os.path.join(
                ...
                split,
                f"match{match_id}",
                "corrected_csv" if split == "test" else "csv",
            ...
            x_pred, y_pred, _vis_pred = (
                np.array(pred_base["X"]),
                np.array(pred_base["Y"]),
                np.array(pred_base["Visibility"]),
            )
            err_type_pred = np.array(pred_base["Type"])

            # Load refined trajectory
            x_refine, y_refine, _vis_refine = (
                np.array(pred_refine["X"]),
                np.array(pred_refine["Y"]),
                np.array(pred_refine["Visibility"]),
            )
            err_type_refine = np.array(pred_refine["Type"])

            for frame_i in range(start_f, end_f):
                if (
                    pred_types_map[err_type_pred[frame_i]] == "FP1"
                    and err_type == "FP1"
                ):
                    cx_true, cy_true = (
                        int(x[frame_i] / w_scaler),
                        int(y[frame_i] / h_scaler),
                    )
                    cx_pred, cy_pred = (
                        int(x_pred[frame_i] / w_scaler),
                        int(y_pred[frame_i] / h_scaler),
                    )
                    diff = math.sqrt(
                    ...
                    pred_types_map[err_type_pred[frame_i]] == "FP2"
                    and err_type == "FP2"
                ):
                    prev_offset, next_offset = 1, 1
                    # Search for nearest visible frame
                    while (
                        frame_i - prev_offset >= 0 and vis[frame_i - prev_offset] != 1
                    ):
                        prev_offset += 1
                    while (
                        frame_i + next_offset < len(x_pred)
                        and vis[frame_i + next_offset] != 1
                    ):
                        next_offset += 1
                    cx_pred, cy_pred = (
                        int(x_pred[frame_i] / w_scaler),
                    ...
                    cx_true, cy_true = (
                        int(x[frame_i - prev_offset] / w_scaler),
                        int(y[frame_i - prev_offset] / h_scaler),
                    )
                    diff_1 = math.sqrt(
                        pow(cx_pred - cx_true, 2) + pow(cy_pred - cy_true, 2)
                    )  # diff with previous frame
                    cx_true, cy_true = (
                        int(x[frame_i + next_offset] / w_scaler),
                        int(y[frame_i + next_offset] / h_scaler),
                    )
                    diff_2 = math.sqrt(
                        pow(cx_pred - cx_true, 2) + pow(cy_pred - cy_true, 2)
                    )  # diff with next frame
            ...
            for frame_i in range(start_f, end_f):
                if (
                    pred_types_map[err_type_refine[frame_i]] == "FP1"
                    and err_type == "FP1"
                ):
                    cx_true, cy_true = (
                        int(x[frame_i] / w_scaler),
                    ...
                    cx_pred, cy_pred = (
                        int(x_refine[frame_i] / w_scaler),
                        int(y_refine[frame_i] / h_scaler),
                    )
                    diff = math.sqrt(
                        pow(cx_pred - cx_true, 2) + pow(cy_pred - cy_true, 2)
                    )
                    ...
                    pred_types_map[err_type_refine[frame_i]] == "FP2"
                    and err_type == "FP2"
                ):
                    prev_offset, next_offset = 1, 1
                    # Search for nearest visible frame
                    while (
                        ...
                        prev_offset += 1
                    while (
                        frame_i + next_offset < len(x_refine)
                        and vis[frame_i + next_offset] != 1
                    ):
                        next_offset += 1
                    cx_pred, cy_pred = (
        ...
        if err_type == "FP1":
            plt.xlabel(
                "Difference between predicted and ground truth coordinate (pixel)"
            )
        else:
            plt.xlabel(
                "Difference between predicted and nearest ground truth coordinate (pixel)"
            )
