...
###################################  Helper Functions ###################################
def get_model(
    model_name: str, seq_len: int | None = None, bg_mode: str | None = None
) -> nn.Module:
...
def to_img_format(input: npt.NDArray, num_ch: int = 1) -> npt.NDArray:
...
def draw_traj(
    img: npt.NDArray, traj: deque, radius: int = 3, color: str = "red"
) -> npt.NDArray:
...
def write_pred_csv(
    pred_dict: dict[str, list[int]], save_file: str, save_inpaint_mask: bool = False
) -> None:
...
def convert_gt_to_coco_json(data_dir: str, split: str, drop: bool = False) -> None:
...
def generate_data_frames(video_file: str) -> None:
...
def get_match_median(match_dir: str) -> None:
...
def get_rally_median(video_file: str) -> None:
...
def re_generate_median_files(data_dir: str) -> None:
