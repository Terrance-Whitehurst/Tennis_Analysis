...
from src.utils.general import ResumeArgumentParser, get_model, to_img_format
...
def get_random_mask(mask_size: tuple[int, int], mask_ratio: float) -> torch.Tensor:
...
def train_inpaintnet(
    ...
    for step, (_, coor_pred, coor_gt, _, vis_gt, _) in enumerate(data_prob):
        ...
        mask = get_random_mask(
            mask_size=coor_gt.shape[:2], mask_ratio=param_dict["mask_ratio"]
        ).cuda()  # (N, L, 1)
        ...
        # Visualize current prediction
        if (step + 1) % display_step == 0:
            coor_gt, refine_coor, inpaint_mask = (
                coor_gt.detach().cpu().numpy(),
                refine_coor.detach().cpu().numpy(),
                inpaint_mask.detach().cpu().numpy(),
            )
            plot_traj_pred_sample(
                coor_gt[0],
                refine_coor[0],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=-1,
        help="alpha of sample mixup, -1 means no mixup",
    )
    parser.add_argument(
        "--frame_alpha",
        type=float,
        default=-1,
        help="alpha of frame mixup, -1 means no mixup",
    )
    parser.add_argument(
        "--mask_ratio",
        type=float,
        default=0.3,
        help="ratio of random mask during training InpaintNet",
    )
    parser.add_argument(
        "--tolerance",
        type=float,
    ...
    parser.add_argument(
        ...
        type=str,
        default="exp",
        help="directory to save the checkpoints and prediction result",
    )
    ...
    print(
        f"TensorBoard: start with 'tensorboard --logdir {os.path.join(args.save_dir, 'logs')}', view at http://localhost:6006/"
    )
    ...
    model = (
        get_model(args.model_name, args.seq_len, args.bg_mode).cuda()
        if args.model_name == "TrackNet"
        else get_model(args.model_name).cuda()
    )
    ...
    if args.lr_scheduler == "StepLR":
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, step_size=int(args.epochs / 3), gamma=0.1
        )
    else:
        scheduler = None
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        print(f"Epoch runtime: {(time.time() - start_time) / 3600.0:.2f} hrs")

    tb_writer.close()
    print(f"Training time: {(time.time() - train_start_time) / 3600.0:.2f} hrs")
    print("Done......")
