"""
Train RF-DETR model for tennis player detection.

RF-DETR (Roboflow Detection Transformer) is a real-time object detection model
that natively accepts COCO-format datasets. Our Player_Detection dataset has
3 classes: players-balls-court (id=0), player-back (id=1), player-front (id=2).

Dataset structure expected:
    data/raw/Player_Detection/
    ├── train/
    │   ├── _annotations.coco.json
    │   └── *.jpg  (5166 images, 1920x1080)
    ├── valid/
    │   ├── _annotations.coco.json
    │   └── *.jpg  (570 images)
    └── test/
...
Usage:
    ...
    python -m src.training.train_player_detection --model large --epochs 100 --batch_size 4
    python -m src.training.train_player_detection --resume models/player_detection/checkpoint.pt
"""
...
def get_args():
    parser = argparse.ArgumentParser(description="Train RF-DETR for player detection")
